...
std::list<Cita> FileIO::getTodasCitas ()
{
    ...
    if (file_pacientes) {
        while (!file_pacientes.eof())
        {
            ...
            //No lee el resto de campos
            for (int i = 0; i < 5; i++)
            {
                file_pacientes.ignore(std::numeric_limits<streamsize>::max(), '\n');
            }

            std::list<Cita> citas_paciente = getCitasPaciente(dni); //Coge las citas del paciente introducido si este existe
...
void FileIO::eliminarPaciente (const Paciente &p)
{
...
void FileIO::archivarPaciente (const Paciente &p)
{
    ...
    if (result >= 0)
    {
        ...
        if (file)
        {
            for (Paciente &paciente : pacientes)
            {
                ...
                //Guardas los datos del paciente
