...
bool nuevoPaciente(Paciente &p, bool modificar)
{
    Paciente pAux;
    std::string aux;
    ...
    if (!modificar)
    {
        //Si añadimos un paciente nuevo pedimos el DNI
    ...
    {
        //Si modificamos un paciente copiamos el DNI
...
bool nuevaCita(Cita &c)
{
    ...
    //Comprobamos si la cita coincide con otra
    std::list<Cita> citas = FileIO::getInstance()->getTodasCitas();
    for (Cita &cita : citas)
    {
        if (coincidenCitas(cAux, cita))
        {
            colorPrint("ERROR: La cita coincide con otra\n", Color::FG_RED, true);
            return false;
        }
    }
...
bool nuevoTratamiento(Tratamiento &t)
{
    Tratamiento tAux;
    std::string aux;
    ...
    //Frecuencia con la que tomar la dosis
    std::cout << "Introduzca cada cuantas horas debe tomar el medicamento: " << std::endl;
    int frecuencia;
    std::cin >> frecuencia;
    ...
    std::cout<<"Introduzca la fecha de comienzo del tratamiento (DD/MM/YYYY):"<<std::endl;
    std::cin >> aux;
    ...
    std::cout<<"Introduzca la fecha de fin del tratamiento (DD/MM/YYYY):"<<std::endl;
    std::cin >> aux;
...
bool nuevaEntradaHistorial(EntradaHistorial &e)
{
...
bool coincidenCitas (const Cita &c1, const Cita &c2)
{
    bool coinciden = true;
...
{
    auto it = str.begin();
    if (*it == '-') { it++; } //Detecta si el numero es negativo
...
bool comprobarFechas (const std::string &f1, const std::string &f2)
{
...
bool comprobarCitas (const Cita &c1, const Cita &c2)
{
...
Selection seleccionarCita (std::list<Cita> citas)
{
    ...
    sortedList.sort(comprobarCitas);
